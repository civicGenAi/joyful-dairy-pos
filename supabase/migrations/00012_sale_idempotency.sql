...
-- 00012: idempotency key for complete_sale(). Needed so the van route's
-- offline sales queue (client-side, src/lib/offline/salesQueue.ts) can safely
-- retry a sale after a dropped connection without risking a double-charge:
-- if the first attempt actually reached the server before the response was
-- lost, the retry with the same client_ref returns the already-created sale
-- instead of inserting a second one.

alter table public.sales add column if not exists client_ref uuid;
create unique index if not exists sales_client_ref_uidx
...
begin
  ...
  insert into public.sales (channel, customer_id, customer_name, payment, tier, location_id, sold_by, date, client_ref)
