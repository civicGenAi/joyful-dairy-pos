...
export const customersRepo = {
  ...
  async create(input: { name: string; phone: string; type: CustomerType }): Promise<Customer> {
    const row = unwrap(
      await supabase
        .from("customers")
        .insert({ name: input.name, phone: input.phone, type: input.type })
        .select("*")
        .single(),
    ) as CustomerRow;
    await recordAudit(
  ...
  async update(input: {
    ...
    unwrap(
      await supabase
        .from("customers")
        .update({ name: input.name, phone: input.phone, type: input.type })
        .eq("id", input.id)
        .select("id"),
    );
    await recordAudit(
