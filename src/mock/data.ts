...
export const CUSTOMERS: Customer[] = customerNames.map((c, i) => {
  ...
  const activities = isMonthly
    ? Array.from({ length: 18 }).map((_, k) => {
        ...
        const qty =
          pid.startsWith("p-mozz") || pid.startsWith("p-ghee") ? 2 + (k % 3) : 4 + (k % 6);
        return {
          id: `a-${i}-${k}`,
          ...
          qty,
          unit:
            pid.startsWith("p-mozz") || pid.startsWith("p-pan") || pid.startsWith("p-hall")
              ? ("kg" as const)
              : pid.startsWith("p-butter") || pid.startsWith("p-yog")
                ? ("pcs" as const)
                : ("L" as const),
          amountTZS: qty * price,
          paid: false,
    ...
    ? [
        {
          id: `d-${i}-1`,
          date: "2026-05-10",
          amountTZS: 150000 + i * 10000,
          ref: `RCT-${1000 + i}A`,
        },
        {
          id: `d-${i}-2`,
          date: "2026-05-22",
          amountTZS: 200000 + i * 8000,
          ref: `RCT-${1000 + i}B`,
        },
      ]
    : c.type === "credit"
...
export const COLLECTIONS_TODAY: CollectionEntry[] = FARMERS.flatMap((f, i) => [
  {
    ...
    date: TODAY,
    session: "morning",
    litres: Math.round((f.litresThisCycle / 15) * 0.6),
    point: i % 2 === 0 ? "field-a" : "main",
  },
  {
    id: `col-${i}-e`,
    farmerId: f.id,
    date: TODAY,
    session: "evening",
    litres: Math.round((f.litresThisCycle / 15) * 0.4),
    point: i % 3 === 0 ? "field-a" : "main",
  },
]);
...
export const ALERTS: Alert[] = [
  {
    id: "al-1",
    kind: "low-stock",
    title: "Butter 250g is out of stock",
    detail: "Last sale 2 days ago, reorder threshold 20 pcs",
    severity: "danger",
    timeAgo: "10 min ago",
  },
  {
    id: "al-2",
    kind: "low-stock",
    title: "Halloumi running low",
    detail: "On hand 4.2 kg, threshold 5 kg",
    severity: "warning",
    timeAgo: "1 hr ago",
  },
  {
    id: "al-3",
    kind: "low-stock",
    title: "Vikopo vya robo low",
    detail: "Balance 140 pcs, threshold 200 pcs",
    severity: "warning",
    timeAgo: "2 hr ago",
  },
  {
    id: "al-4",
    kind: "overdue-credit",
    title: "Mamis Bistro overdue",
    detail: "TZS 420,000 overdue 12 days",
    severity: "danger",
    timeAgo: "Today",
  },
  {
    id: "al-5",
    kind: "overdue-credit",
    title: "Walter Sanare overdue",
    detail: "TZS 38,500 overdue 18 days",
    severity: "warning",
    timeAgo: "Today",
  },
  {
  ...
  },
  {
    id: "al-7",
    kind: "day-unbalanced",
    title: "Yesterday not yet locked",
    detail: "Production manager to confirm day-close",
    severity: "warning",
    timeAgo: "Yesterday",
  },
];
...
export const NAV_GROUPS_BY_ROLE: Record<
  string,
  { group: string; sw: string; items: { to: string; label: string; sw: string; icon: string }[] }[]
> = {
  admin: [
    {
      group: "Overview",
      sw: "Muhtasari",
      items: [{ to: "/dashboard", label: "Dashboard", sw: "Dashibodi", icon: "LayoutDashboard" }],
    },
    {
      group: "Procurement",
      sw: "Ununuzi",
    ...
    },
    {
      group: "System",
      sw: "Mfumo",
      items: [{ to: "/settings", label: "Settings", sw: "Mipangilio", icon: "Settings" }],
    },
  ],
  finance: [
    {
    ...
    },
    {
      group: "Insights",
      sw: "Ripoti",
      items: [{ to: "/reports", label: "Reports", sw: "Ripoti", icon: "BarChart3" }],
    },
  ],
  production: [
    {
  ...
  route: [
    {
      group: "Route",
      sw: "Njia",
      items: [{ to: "/van", label: "Route module", sw: "Njia", icon: "Truck" }],
    },
  ],
  store: [
    {
      group: "Store",
      sw: "Ghala",
      items: [{ to: "/stock", label: "Stock & store", sw: "Ghala", icon: "Boxes" }],
    },
  ],
  viewer: [
    {
