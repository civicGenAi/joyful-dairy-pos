...
import { ALERTS, FARMERS, MILK_TREND_30, SALES_BY_CATEGORY_WEEK, SALES_CHANNEL_SPLIT, TOP_CUSTOMERS, YIELD_WEEK } from "@/mock/data";
import {
    ...
    <AppShell title={t("Dashibodi", "Dashboard")}>
      ...
      <div className="relative overflow-hidden rounded-3xl p-6 lg:p-8 text-white shadow-elevated mb-6"
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="text-[11px] uppercase tracking-[0.2em] opacity-80">{new Date().toLocaleDateString(lang === "sw" ? "sw-TZ" : "en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <StatCard label={t("Yaliyokusanywa leo", "Collected today")} value={<><CountUp value={882} /> L</>} sub={<><TrendingUp className="inline h-3 w-3 text-[#2F9E44]" /> +4.2% vs jana</>} accent="green" />
        ...
        <StatCard label={t("Yaliyoharibika", "Spoilt today")} value={<><CountUp value={12} /> L</>} sub={<><TrendingDown className="inline h-3 w-3 text-[#2F9E44]" /> -1.5 L vs jana</>} accent="red" />
        ...
        <StatCard label={t("Mapato leo", "Revenue today")} value={<CountUp value={1428000} format={(v) => tzs(v)} />} sub={t("Pamoja na wateja wa mkopo", "Includes credit clients")} accent="green" />
        ...
        <StatCard label={t("Malipo wafugaji", "Farmer payables")} value={<CountUp value={4830000} format={(v) => tzs(v)} />} sub={t("Mzunguko unaomalizika tar. 15 Juni", "Next 15-day cycle: Jun 15")} accent="info" />
        ...
        <SectionCard title={t("Hali ya stock", "Stock health")}>
          <div className="space-y-3">
            {[
              ...
              <div key={s.name}>
                <div className="flex justify-between text-xs mb-1"><span>{s.name}</span><Pill tone={s.tone}>{num(s.v)}</Pill></div>
