...
import { Plus, Minus, Trash2, Printer, ShoppingBasket, AlertTriangle, Receipt as ReceiptIcon, History, Pause, CheckCircle2 } from "lucide-react";
...
interface CompletedSale { id: string; total: number; lines: CartLine[]; customer: string; payment: string; at: string; }

// Demo "my shift" history.
const SHIFT_SEED: CompletedSale[] = [
  { id: "RCT-2105", total: 18000, lines: [{ productId: "p-fresh", qty: 12, tier: "own" }], customer: "walkin", payment: "cash", at: "08:42" },
  { id: "RCT-2106", total: 12000, lines: [{ productId: "p-mtindi", qty: 6, tier: "own" }], customer: "walkin", payment: "mpesa", at: "09:15" },
  { id: "RCT-2107", total: 42000, lines: [{ productId: "p-mozz", qty: 3, tier: "bulk" }], customer: "c1", payment: "credit", at: "10:30" },
];

const ORDERS_SEED = [
  { id: 1001, customerId: "c1", customer: "Mamis Bistro", productId: "p-mozz", product: "Mozzarella", qty: "3 kg", amount: 42000, status: "issued" as const },
  { id: 1002, customerId: "c15", customer: "CSR Hotel", productId: "p-fresh", product: "Fresh milk", qty: "40 L", amount: 60000, status: "issued" as const },
  { id: 1003, customerId: "c13", customer: "Jovinary Hotel", productId: "p-mtindi", product: "Mtindi", qty: "30 L", amount: 51000, status: "pending" as const },
  { id: 1004, customerId: "c6", customer: "Sidhu Restaurant", productId: "p-yog-van", product: "Yoghurt Vanilla", qty: "24 pcs", amount: 60000, status: "issued" as const },
];
...
export function POSScreen() {
  ...
  const completeSale = () => {
    ...
    const sale: CompletedSale = {
      ...
      total,
      lines: cart,
      customer: customer === "walkin" ? (lang === "sw" ? "Mteja wa kupita" : "Walk-in") : (CUSTOMERS.find((c) => c.id === customer)?.name ?? customer),
      payment: pay,
  ...
  return (
    <AppShell title={t("Mauzo ya Kaunta", "Counter POS")}>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <StatCard label={t("Mapato kipindi changu", "My shift revenue")} value={tzs(todaySales)} sub={`${shift.length} ${t("mauzo", "sales")}`} accent="green" />
      ...
      <Tabs defaultValue="pos">
          ...
          <div className="grid lg:grid-cols-3 gap-4">
              ...
              <div className="p-4 grid grid-cols-2 md:grid-cols-3 gap-3">
                {products.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <div className="aspect-[16/9] rounded-lg mb-2 relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${CATS.find((c) => c.id === cat)?.color}30, ${CATS.find((c) => c.id === cat)?.color}10)` }}>
                        <div className="absolute inset-0 grid place-items-center text-3xl">{p.unit === "L" ? "🥛" : p.unit === "kg" ? "🧀" : "🥤"}</div>
                      </div>
                      ...
                      <div className="mt-1.5 flex items-center justify-between">
                        ...
                        <Pill tone={out ? "danger" : low ? "warning" : "success"}>{out ? t("Imeisha", "Out") : low ? `${num(onHand)} ${p.unit}` : p.unit}</Pill>
                      </div>
                    </motion.button>
                  );
                })}
            ...
            <div className="rounded-2xl border border-border bg-card shadow-card flex flex-col">
              <div className="px-4 py-3 border-b border-border font-display font-semibold flex items-center justify-between gap-2">
                <span className="flex items-center gap-2"><ShoppingBasket className="h-4 w-4" />{t("Mkokoteni", "Cart")} ({itemCount})</span>
                {parked.length > 0 && (
                  <ParkedDropdown parked={parked} onRestore={(idx) => { setCart(parked[idx]); setParked((p) => p.filter((_, i) => i !== idx)); }} />
                )}
              </div>
              <div className="p-3 border-b border-border space-y-2">
                ...
                {selectedCustomer && selectedCustomer.outstandingTZS > 0 && (
                  <div className="rounded-lg bg-secondary/60 px-2 py-1.5 text-[11px] flex justify-between">
                    <span className="text-muted-foreground">{t("Deni la sasa", "Open balance")}</span>
                    <span className="font-num font-semibold">{tzs(selectedCustomer.outstandingTZS)}</span>
                  </div>
                )}
                ...
                {isCreditBlocked && (
                  <div className="rounded-lg bg-[#E11B22]/10 text-[#E11B22] px-2 py-1.5 text-[11px] flex items-center gap-1.5">
                    ...
                    {t("Mteja amechelewa, anaweza tu kulipa cash au mpesa.", "Customer is overdue, accept only cash or mpesa.")}
                  </div>
                )}
              </div>
              ...
              <div className="border-t border-border p-3 space-y-2">
                <div className="flex justify-between text-sm"><span>{t("Bidhaa", "Items")}</span><span className="font-num">{itemCount}</span></div>
                ...
                <div className="flex gap-2">
                  <Button variant="outline" disabled={!cart.length} onClick={park} title={t("Weka pembeni", "Park")}>
                    <Pause className="h-4 w-4" />
                  </Button>
                  <Button disabled={cart.length === 0 || isCreditBlocked} onClick={completeSale} className="flex-1 h-11 text-white" style={{ background: "linear-gradient(135deg, #1E7C3F, #8CC63F)" }}>
          ...
          >
            <table className="w-full text-sm">
              <thead><tr className="text-left text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border">
                <th className="py-2 px-2">#</th><th>{t("Mteja", "Customer")}</th><th>{t("Bidhaa", "Product")}</th><th className="text-right">{t("Idadi", "Qty")}</th><th className="text-right">{t("Jumla", "Total")}</th><th>{t("Hali", "Status")}</th><th />
              </tr></thead>
              <tbody>
                {orders.map((r) => (
                  <tr key={r.id} className="border-b border-border last:border-0">
                    ...
                    <td className="py-2.5"><Pill tone={r.status === "issued" ? "success" : "warning"}>{r.status}</Pill></td>
                    <td className="py-2.5 text-right">
                      {r.status === "pending" && (
                        <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => { setOrders((xs) => xs.map((x) => x.id === r.id ? { ...x, status: "issued" } : x)); toast.success(t("Order imetolewa", "Order issued")); }}>
        ...
        <TabsContent value="history" className="mt-4">
          <SectionCard title={t("Mauzo ya kipindi changu", "My shift sales")} action={
            <Pill tone="info">{user?.name.split(" ")[0]} · {TODAY}</Pill>
          }>
            <table className="w-full text-sm">
              <thead><tr className="text-left text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border">
                <th className="py-2 px-3">{t("Wakati", "Time")}</th><th>{t("Risiti", "Receipt")}</th><th>{t("Mteja", "Customer")}</th><th>{t("Malipo", "Payment")}</th><th className="text-right">{t("Jumla", "Total")}</th><th />
              </tr></thead>
              <tbody>
                {shift.map((s) => (
                  <tr key={s.id} className="border-b border-border last:border-0">
                    ...
                    <td className="py-2.5"><Pill tone={s.payment === "cash" ? "success" : s.payment === "credit" ? "warning" : "info"}>{s.payment}</Pill></td>
                    <td className="py-2.5 text-right font-num font-semibold">{tzs(s.total)}</td>
                    <td className="py-2.5 text-right">
                      <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => nav({ to: "/receipt/$id", params: { id: s.id } })}>
                        <Printer className="h-3.5 w-3.5 mr-1" />{t("Chapisha", "Print")}
                      </Button>
                    </td>
                  </tr>
                ))}
...
function ParkedDropdown({ parked, onRestore }: { parked: CartLine[][]; onRestore: (i: number) => void }) {
  const { t } = useApp();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="h-7 text-xs">
          <History className="h-3.5 w-3.5 mr-1" /> {parked.length} {t("zimewekwa pembeni", "parked")}
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>{t("Mauzo yaliyowekwa pembeni", "Parked sales")}</DialogTitle></DialogHeader>
          ...
          {parked.map((p, i) => {
            ...
            return (
              <li key={i} className="flex items-center justify-between py-3">
                <div>
                  <div className="font-medium text-sm">{p.length} {t("vifurushi", "items")}</div>
                  <div className="text-xs text-muted-foreground">{p.map((l) => PRODUCTS.find((x) => x.id === l.productId)?.name).join(", ")}</div>
                </div>
                <div className="text-right">
                  <div className="font-num font-bold">{tzs(total)}</div>
                  <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => { onRestore(i); setOpen(false); }}>{t("Endelea", "Restore")}</Button>
                </div>
              </li>
            );
          })}
...
function NewOrderDialog({ onAdd }: { onAdd: (o: typeof ORDERS_SEED[number]) => void }) {
  ...
  const save = () => {
    ...
    onAdd({
      ...
      customerId, customer: cust.name,
      productId, product: prod.name,
      qty: `${qty} ${prod.unit}`,
      amount: price,
      status: "pending",
    });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader><DialogTitle className="flex items-center gap-2"><ReceiptIcon className="h-4 w-4" />{t("Unda order ya kutoa kwa stock", "Create stock-issue order")}</DialogTitle></DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={save} className="text-white" style={{ background: "linear-gradient(135deg, #1E7C3F, #8CC63F)" }}>{t("Unda", "Create")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
