...
import { Wifi, WifiOff, Sun, Moon, Truck, Plus, Minus, Receipt, RotateCcw, LogOut, Search, ArrowRight, MapPin, Printer, CheckCircle2 } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

type Tier = "own" | "bottle" | "bulk";
interface CartLine { productId: string; qty: number; tier: Tier; }
interface RouteSale { id: string; at: string; customerId: string; customer: string; lines: CartLine[]; total: number; payment: "cash" | "credit" | "mpesa"; }
...
export function RouteScreen() {
  ...
  const customerMatches = useMemo(() =>
    CUSTOMERS.filter((c) => !customerQ || c.name.toLowerCase().includes(customerQ.toLowerCase())).slice(0, 6)
  , [customerQ]);
  ...
  const completeSale = () => {
    ...
    const sale: RouteSale = {
      ...
      customerId: customer, customer: cust.name,
      lines: cart, total: cartTotal, payment,
    };
    setSales((xs) => [sale, ...xs]);
        ...
        <div className="mx-auto max-w-md flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={online ? t("Bonyeza kuiga offline", "Click to simulate offline") : t("Bonyeza kurudi online", "Click to go online")}
            >
      ...
      <main className="mx-auto max-w-md px-4 py-5 pb-28 space-y-4">
        <div className="rounded-2xl text-white p-4 shadow-elevated" style={{ background: "linear-gradient(135deg, #14532D, #1E7C3F, #2F9E44)" }}>
          ...
          <div className="mt-2 text-[11px] opacity-90">
            {t("Wateja waliotembelewa", "Customers visited")}: <span className="font-num font-bold">{visited.size}/{routeStops.length}</span>
          </div>
        </div>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="plan" className="space-y-3 mt-4">
            <div className="rounded-2xl bg-card border border-border p-4">
              <div className="font-semibold mb-3 flex items-center gap-2"><MapPin className="h-4 w-4" />{t("Ratiba ya leo", "Today's route")}</div>
              <ul className="space-y-2">
                {routeStops.map((cid, i) => {
                  const c = CUSTOMERS.find((x) => x.id === cid)!;
                  const done = visited.has(cid);
                  return (
                    <li key={cid} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 ${done ? "bg-[#2F9E44]/10" : "bg-secondary/60"}`}>
                      <span className={`grid h-7 w-7 place-items-center rounded-full text-xs font-bold ${done ? "bg-[#1E7C3F] text-white" : "bg-background text-foreground"}`}>{done ? <CheckCircle2 className="h-3.5 w-3.5" /> : i + 1}</span>
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-sm truncate">{c.name}</div>
                        <div className="text-[11px] text-muted-foreground">{c.phone}</div>
                      </div>
                      <Button size="sm" variant={done ? "outline" : "default"} className={done ? "" : "text-white"} style={done ? undefined : { background: "linear-gradient(135deg, #1E7C3F, #8CC63F)" }} onClick={() => { setCustomer(cid); setTab("sell"); }}>
                        {done ? t("Tena", "Again") : t("Uza", "Sell")} <ArrowRight className="h-3 w-3 ml-1" />
                      </Button>
                    </li>
                  );
                })}
                    ...
                    <li key={l.productId} className="rounded-xl bg-secondary/60 px-3 py-2.5">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <div className="font-num font-bold">{num(remaining)} / {num(l.litres)} {p.unit}</div>
                          <div className="text-[10px] text-muted-foreground">{t("Imeuzwa", "Sold")}: {num(sold)}</div>
                        </div>
                      </div>
                      <div className="mt-2 h-1.5 rounded-full bg-background overflow-hidden">
                        <div className="h-full" style={{ background: "linear-gradient(90deg, #1E7C3F, #8CC63F)", width: `${(remaining / l.litres) * 100}%` }} />
                      </div>
                    </li>
                  );
                })}
          ...
          <TabsContent value="sell" className="space-y-3 mt-4">
            <div className="rounded-2xl bg-card border border-border p-4 space-y-3">
              <div className="grid gap-1.5">
                ...
                <div className="relative">
                  ...
                  <Input value={customerQ} onChange={(e) => setCustomerQ(e.target.value)} className="pl-8 h-9" placeholder={t("Tafuta mteja…", "Search customer…")} />
                </div>
                <Select value={customer} onValueChange={setCustomer}>
                  <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {customerMatches.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="grid grid-cols-3 gap-2">
                {ROUTE_LOAD.map((l) => {
                  ...
                  return (
                    <button key={l.productId} disabled={remaining === 0} onClick={() => addToCart(l.productId)} className={`rounded-xl border border-border bg-background p-3 text-left ${remaining === 0 ? "opacity-50" : "hover:border-[#2F9E44]"}`}>
                      <div className="text-xs font-semibold">{p.name}</div>
                      <div className="font-num text-sm font-bold mt-1">{num(PRICE_MATRIX[p.id].own)}</div>
                      <div className="text-[10px] text-muted-foreground mt-1">{num(remaining)} {p.unit} {t("zinabakia", "left")}</div>
                    </button>
                  );
                })}
              </div>
              ...
              <Button disabled={cart.length === 0} className="w-full h-11 text-white" style={{ background: "linear-gradient(135deg, #1E7C3F, #8CC63F)" }} onClick={completeSale}>
            ...
            {sales.length > 0 && (
              <div className="rounded-2xl bg-card border border-border p-4">
                <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">{t("Mauzo ya leo", "Today's sales")}, {sales.length}</div>
                <ul className="divide-y divide-border text-sm">
                  {sales.slice(0, 5).map((s) => (
                    <li key={s.id} className="flex items-center justify-between py-2">
                      <div>
                        <div className="font-medium">{s.customer}</div>
                        <div className="text-xs text-muted-foreground">{s.at} · {s.payment}</div>
                      </div>
          ...
          <TabsContent value="returns" className="space-y-3 mt-4">
            <div className="rounded-2xl bg-card border border-border p-4 space-y-3">
                  ...
                  <div key={l.productId} className="rounded-xl bg-secondary/60 p-2.5">
                    <div className="flex items-center justify-between mb-2 text-sm">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{t("Iliyobakia kwenye gari", "Remaining on van")}: {num(remaining)} {p.unit}</div>
                      </div>
                      <Button size="sm" variant="outline" onClick={() => setReturned((r) => ({ ...r, [l.productId]: remaining }))}>
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) => setReturned((r) => ({ ...r, [l.productId]: Number(e.target.value) }))}
                      />
              ...
              <Button variant="outline" className="w-full" onClick={() => toast.success(t(`${totalReturned} units zimerekodi`, `${totalReturned} units saved`))}>{t("Hifadhi", "Save returns")}</Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="cashup" className="space-y-3 mt-4">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={k as string} className="border-b border-border last:border-0">
                      ...
                      <td className="py-2 text-right font-num font-semibold">{v} {suf}</td>
                    </tr>
                  ))}
                  <tr><td className="py-2 font-semibold">{t("Cash inayotarajiwa", "Expected deposit")}</td><td className="py-2 text-right font-num font-bold">{tzs(expectedDeposit)}</td></tr>
                </tbody>
              </table>
              <div className="grid gap-1.5">
                <Label>{t("Pesa iliyowekwa kwa Finance", "Deposited to finance (TZS)")}</Label>
                <Input type="number" value={deposit} onChange={(e) => setDeposit(Number(e.target.value))} className="font-num" />
              </div>
              {deposit > 0 && deposit !== expectedDeposit && (
                <div className="rounded-xl bg-[#E5A100]/10 text-[#8a5a00] text-xs px-3 py-2">
                  {t("Tofauti", "Difference")}: {tzs(deposit - expectedDeposit)} {t("(eleza kwenye notes)", "(explain in notes)")}
                </div>
              )}
              <Button className="w-full text-white" style={{ background: "linear-gradient(135deg, #1E7C3F, #8CC63F)" }} onClick={generateCashUp}>
                <Printer className="h-4 w-4 mr-1.5" />{t("Tengeneza na chapisha", "Generate & print")}
              </Button>
              <AnimatePresence>
                {deposit > 0 && (
            ...
            {!online && (
              <div className="rounded-xl bg-secondary/60 px-3 py-2 text-xs text-muted-foreground flex items-center gap-2">
                <WifiOff className="h-3.5 w-3.5" />{t("Data inahifadhiwa kwenye simu, itasawazishwa ukirudi online.", "Saved on device, will sync when you return online.")}
              </div>
            )}
          </TabsContent>
        </Tabs>
