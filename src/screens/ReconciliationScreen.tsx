...
export function ReconciliationScreen() {
  ...
  const setRow = (i: number, k: keyof Row, v: number) => setRows((xs) => xs.map((r, idx) => idx === i ? { ...r, [k]: v } : r));
  ...
  return (
    <AppShell title={t("Sawazisha siku, Funga siku", "Day reconciliation & day-close")}>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        ...
        <StatCard label={t("Zilizosawazi", "Balanced")} value={`${rows.filter(isBalanced).length}/${rows.length}`} accent={allBalanced ? "green" : "amber"} />
      ...
      {locked && (
        <div className="rounded-2xl border border-[#1D9E75]/30 bg-[#1D9E75]/10 p-4 mb-5 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-semibold text-sm">{t("Siku imefungwa, kusoma tu", "Day locked, read-only")}</div>
          ...
          <Button asChild variant="outline" className="border-[#1E7C3F] text-[#1E7C3F]">
            <Link to="/report/day-close/$date" params={{ date: TODAY }}>
              <FileText className="h-3.5 w-3.5 mr-1.5" />{t("Ripoti ya kufunga", "Open report")}
            </Link>
          </Button>
        </div>
      )}

      <SectionCard title={t("Sawazisho la kila bidhaa", "Per-product reconciliation")} action={
        <div className="flex gap-2">
          <ExportMenu formats={["pdf"]} filename={`day-reconciliation-${TODAY}`} />
          <Button asChild variant="outline" size="sm" className="h-8 text-xs">
            <Link to="/report/day-close/$date" params={{ date: TODAY }}>
              <FileText className="h-3.5 w-3.5 mr-1.5" />{t("Tazama ripoti", "Preview report")}
            </Link>
          </Button>
        </div>
      }>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r, i) => {
                ...
                const cell = (k: keyof Row, value: number) => editable ? (
                  <Input
                    type="number"
                    value={value}
                    step={value % 1 ? 0.05 : 1}
                ...
                return (
                  <tr key={i} className={`border-b border-border last:border-0 hover:bg-accent/40 ${!ok ? "bg-[#E11B22]/5" : ""}`}>
                    ...
                    <td className="py-1 px-2 text-right text-[#E11B22]">{cell("spoilt", r.spoilt)}</td>
                    ...
                    <td className="py-2.5 px-2 text-center" title={ok ? "" : `Δ ${num(balanceOf(r).delta, 2)}`}>
                      {ok ? <Check className="inline h-4 w-4 text-[#2F9E44]" /> : (
                        <span className="inline-flex items-center gap-1 text-[#E11B22] text-[11px] font-num font-semibold"><X className="h-3.5 w-3.5" />Δ {num(balanceOf(r).delta, 2)}</span>
                      )}
                    </td>
                  </tr>
                );
        ...
        {!allBalanced && (
          <div className="mt-3 rounded-xl bg-[#E11B22]/5 border border-[#E11B22]/30 p-3 text-xs text-[#8c0d12]">
            {t("Mistari iliyo nyekundu haijasawazi. Hariri opening, collected, produced, sold, spoilt au closing ili Δ iwe 0.", "Red rows do not balance. Edit opening, collected, produced, sold, spoilt or closing until Δ is 0.")}
          </div>
        )}
      </SectionCard>
      ...
      <div className="mt-5 grid lg:grid-cols-2 gap-4">
        <SectionCard title={t("Vyanzo vya maziwa ghafi leo", "Raw milk sources today")}>
          <ul className="divide-y divide-border">
            ...
            <li className="flex justify-between py-3 font-bold border-t-2 border-[#1E7C3F]/40"><span>{t("Jumla", "Total")}</span><span className="font-num text-[#1E7C3F]">{num(sourcesTotal)} L</span></li>
          </ul>
          <div className="mt-3 text-xs text-muted-foreground">
            {t("Imejumlishwa kutoka kwa wafugaji", "Aggregated from")} {FARMERS.length} {t("wakichukua mizigo miwili kwa siku.", "farmers across two daily sessions.")}
          </div>
        </SectionCard>

        <SectionCard title={t("Funga siku", "Day close")}>
          <p className="text-sm text-muted-foreground mb-4">
            {t("Baada ya kufunga, salio la mwisho linakuwa salio la awali la kesho. Inahitajika jukumu la Production au Admin.", "After locking, closing balances become tomorrow's opening. Requires Production Manager or Admin role.")}
          </p>
          <div className="grid grid-cols-3 gap-2 mb-4">
            {rows.slice(0, 3).map((r) => (
              <div key={r.product} className="rounded-xl bg-secondary/60 p-2.5">
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground truncate">{r.product}</div>
                <div className="font-num font-bold">{num(r.closing, r.closing % 1 ? 2 : 0)}</div>
                <div className="text-[10px] text-muted-foreground">{t("Salio la awali la kesho", "Tomorrow's opening")}</div>
              </div>
            ))}
          </div>
          <div className="flex gap-2">
            <ConfirmDialog
              ...
              onConfirm={() => { if (!allBalanced) { toast.error(t("Bidhaa moja au zaidi haijasawazi.", "One or more products are unbalanced.")); return; } setLocked(true); toast.success(t("Siku imefungwa", "Day locked")); }}
              trigger={
                <Button disabled={!canLock || locked || !allBalanced} className="text-white" style={{ background: "linear-gradient(135deg, #1E7C3F, #8CC63F)" }}>
                  <Lock className="h-4 w-4 mr-1.5" />{locked ? t("Imefungwa", "Locked") : t("Funga siku", "Lock day")}
                </Button>
              }
            />
            {!canLock && <Pill tone="warning">{t("Huna ruhusa", "No permission")}</Pill>}
            {!allBalanced && canLock && !locked && <Pill tone="danger">{t("Sawazisha kwanza", "Resolve mismatches")}</Pill>}
          </div>
        </SectionCard>
      </div>

      <div className="mt-5">
        <SectionCard title={t("Historia ya kufunga siku", "Past day-closes")} action={<History className="h-4 w-4 text-muted-foreground" />}>
          <ul className="divide-y divide-border">
            {PAST_LOCKS.map((p) => (
              <li key={p.date} className="flex items-center gap-3 py-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">{t("Imefungwa na", "Locked by")} {p.by}</div>
                </div>
                <Pill tone={p.confirmed ? "success" : "warning"}>{p.confirmed ? t("Imethibitishwa", "Confirmed") : t("Inasubiri Finance", "Awaiting finance")}</Pill>
                <Button asChild size="sm" variant="ghost" className="h-7 text-xs">
                  <Link to="/report/day-close/$date" params={{ date: p.date }}>
                    <FileText className="h-3.5 w-3.5 mr-1" />{t("Ripoti", "Report")}
                  </Link>
                </Button>
              </li>
            ))}
