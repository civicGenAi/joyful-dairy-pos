...
import { AreaChart, Area, BarChart, Bar, LineChart, Line, ResponsiveContainer, XAxis, YAxis, Tooltip, CartesianGrid, PieChart, Pie, Cell, Legend } from "recharts";
import { MILK_TREND_30, SALES_CHANNEL_SPLIT, TOP_CUSTOMERS, FARMERS, TODAY, TODAY_LABEL } from "@/mock/data";
      ...
      <Tabs defaultValue="daily">
        ...
        <TabsContent value="daily">
          <div className="flex items-center gap-2 mb-4">
            <Input type="date" value={dailyDate} onChange={(e) => setDailyDate(e.target.value)} className="h-9 w-44 text-sm" />
            <Button asChild variant="outline" size="sm">
              <Link to="/report/day-close/$date" params={{ date: dailyDate }}>
                <FileText className="h-3.5 w-3.5 mr-1.5" />{t("Ripoti ya kufunga", "Day-close report")}
              </Link>
            </Button>
          </div>
          ...
          <SectionCard title={`${t("Ripoti ya kila siku", "Daily report")}, ${dailyDate === TODAY ? TODAY_LABEL : dailyDate}`}>
        ...
        <TabsContent value="weekly">
          <div className="flex items-center gap-2 mb-4">
            <span className="text-xs text-muted-foreground">{t("Wiki inayoanza", "Week starting")}</span>
            <Input type="date" value={weeklyStart} onChange={(e) => setWeeklyStart(e.target.value)} className="h-9 w-44 text-sm" />
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          ...
          <div className="mt-4 grid lg:grid-cols-2 gap-4">
            <SectionCard title={t("Wateja wa juu, wiki", "Top customers, week")}>
              <table className="w-full text-sm">
                <tbody>{TOP_CUSTOMERS.slice(0, 6).map((c, i) => (
                  <tr key={c.name} className="border-b border-border last:border-0">
                    <td className="py-2 px-3 font-num text-xs text-muted-foreground">{i + 1}</td>
                    <td className="py-2 px-3 font-medium">{c.name}</td>
                    <td className="py-2 px-3 text-right font-num font-semibold">{tzs(c.value / 4)}</td>
                  </tr>
                ))}</tbody>
              </table>
            </SectionCard>
            <SectionCard title={t("Wafugaji wa juu, wiki", "Top farmers, week")}>
              <table className="w-full text-sm">
                <tbody>{FARMERS.slice().sort((a, b) => b.litresThisCycle - a.litresThisCycle).slice(0, 6).map((f, i) => (
                  <tr key={f.id} className="border-b border-border last:border-0">
                    <td className="py-2 px-3 font-num text-xs text-muted-foreground">{i + 1}</td>
                    <td className="py-2 px-3 font-medium">{f.name}</td>
                    <td className="py-2 px-3 text-right font-num font-semibold">{L(Math.round(f.litresThisCycle / 2))}</td>
                  </tr>
                ))}</tbody>
              </table>
            </SectionCard>
        ...
        <TabsContent value="monthly">
          <div className="flex items-center gap-2 mb-4">
            ...
            <Select value={monthlyMonth} onValueChange={setMonthlyMonth}>
              ...
              <SelectContent>
                {["2026-05", "2026-04", "2026-03", "2026-02", "2026-01"].map((m) => <SelectItem key={m} value={m}>{m}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          ...
          <div className="grid lg:grid-cols-3 gap-4">
            <SectionCard title={t("Mauzo kwa wiki, Mei", "Sales by week, May")} className="lg:col-span-2">
            ...
            <SectionCard title={t("Mafanikio", "Highlights")}>
              <ul className="space-y-3 text-sm">
                <li className="flex justify-between"><span className="text-muted-foreground">{t("Mauzo bora ya siku", "Best sales day")}</span><span className="font-num font-semibold">22 Mei, {tzs(1880000)}</span></li>
                <li className="flex justify-between"><span className="text-muted-foreground">{t("Mteja wa juu", "Top customer")}</span><span className="font-semibold">Mamis Bistro</span></li>
                <li className="flex justify-between"><span className="text-muted-foreground">{t("Mfugaji wa juu", "Top farmer")}</span><span className="font-semibold">Idda Kirenga</span></li>
                <li className="flex justify-between"><span className="text-muted-foreground">{t("Yield wastani", "Avg yield")}</span><span className="font-num">83.6%</span></li>
                <li className="flex justify-between"><span className="text-muted-foreground">{t("Spoilage rate", "Spoilage")}</span><span className="font-num">1.6%</span></li>
              </ul>
            </SectionCard>
          </div>
        </TabsContent>
        ...
        <TabsContent value="schedule">
          <div className="grid lg:grid-cols-3 gap-4">
            <SectionCard title={t("Utumaji wa ripoti otomatiki", "Scheduled report delivery")} className="lg:col-span-2">
              <table className="w-full text-sm">
                    ...
                    <tr key={r.name} className="border-b border-border last:border-0">
                      ...
                      <td className="py-2.5 text-right">
                        <div className="flex gap-1 justify-end">
                          <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setPreviewing({ recipient: r.name, channel: r.ch })}>
                            <Eye className="h-3.5 w-3.5 mr-1" />{t("Hakikisho", "Preview")}
                          </Button>
                          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => toast.success(t(`Imetumwa kwa ${r.ch}`, `Sent via ${r.ch}`))}>
                            <Send className="h-3.5 w-3.5 mr-1" />{t("Tuma sasa", "Send")}
                          </Button>
                        </div>
                      </td>
                    </tr>
            ...
            <SectionCard title={t("Hakikisho la ripoti", "Report preview")}>
              {!previewing ? (
                <div className="text-sm text-muted-foreground text-center py-12">
                  {t("Bofya Hakikisho kwenye mtu yeyote ili kuona ripoti kabla ya kutuma.", "Click Preview on any recipient to see the report before sending.")}
                </div>
              ) : (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="text-xs uppercase tracking-wider text-muted-foreground">{previewing.channel}</div>
                      <div className="font-semibold text-sm">{previewing.recipient}</div>
                    </div>
                    <button onClick={() => setPreviewing(null)} className="rounded-md p-1.5 hover:bg-accent"><X className="h-4 w-4" /></button>
                  </div>
                  <div className="rounded-xl border border-dashed border-border p-3 text-xs space-y-1.5 font-num">
                    <div className="font-display font-bold text-sm">{previewing.channel === "SMS" ? "AFR JOY DAILY" : t("Muhtasari wa siku, African Joy", "Daily summary, African Joy")}</div>
                    <div>{TODAY_LABEL}</div>
                    <div>{t("Mauzo", "Sales")}: {tzs(1428000)}</div>
                    <div>{t("Litre", "Litres")}: 520 L</div>
                    <div>{t("Yaliyoharibika", "Spoilt")}: 17 L</div>
                    <div>{t("Wateja wa juu", "Top customer")}: Mamis Bistro</div>
                    <div>{t("Hali ya siku", "Day")}: {t("Imefungwa", "Locked")}</div>
                    {previewing.channel === "SMS" ? null : (
                      <div className="pt-2 mt-2 border-t border-border text-[10px] text-muted-foreground">
                        {t("Ripoti kamili ipo kwenye PDF iliyoambatishwa.", "Full report attached as PDF.")}
                      </div>
                    )}
                  </div>
                  <Button onClick={() => toast.success(t(`Imetumwa kwa ${previewing.channel}`, `Sent via ${previewing.channel}`))} className="w-full mt-3 text-white" style={{ background: "linear-gradient(135deg, #1E7C3F, #8CC63F)" }}>
                    <Send className="h-3.5 w-3.5 mr-1.5" />{t("Tuma sasa", "Send now")}
                  </Button>
                </div>
              )}
            </SectionCard>
