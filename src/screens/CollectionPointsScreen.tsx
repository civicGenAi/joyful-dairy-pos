...
interface Transfer { id: string; from: string; to: string; litres: number; time: string; }
  ...
  return (
    <AppShell title={t("Pointi za ukusanyaji", "Collection points")}>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        ...
        <StatCard label={t("Wahamishaji", "Transfers")} value={num(transfers.length)} sub={t("Pointi → Plant → Van", "Point → Plant → Van")} accent="amber" />
      </div>

      <div className="grid lg:grid-cols-2 gap-4 mb-5">
        {[
          { id: "field-a" as const, name: "Point A, Olasiti", swName: "Pointi A, Olasiti", intake: pointA },
          { id: "main" as const, name: "Main plant, Arusha", swName: "Kiwandani, Arusha", intake: main },
        ].map((p) => (
          <div key={p.id} className="rounded-2xl overflow-hidden border border-border shadow-card bg-card">
            ...
            <div className="p-4 flex gap-2">
              <TransferDialog from={p.name} onSave={(tr) => setTransfers((xs) => [{ ...tr, id: `tr-${Date.now()}`, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }, ...xs])} />
              <Button variant="outline" className="rounded-xl" onClick={() => setViewing(p.id)}>
                <Eye className="h-3.5 w-3.5 mr-1.5" />{t("Tazama matokeo", "View intake log")}
              </Button>
            </div>
          </div>
        ))}
      ...
      <SectionCard title={`${t("Matokeo ya leo, daftari", "Today's intake log")} (${TODAY})`} action={<ScrollText className="h-4 w-4 text-muted-foreground" />}>
    ...
    <Sheet open onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        ...
        <div className="mt-5 grid grid-cols-4 gap-2">
          <div className="rounded-xl bg-secondary/60 p-3"><div className="text-[10px] uppercase tracking-wider text-muted-foreground">{t("Jumla", "Total")}</div><div className="font-num font-bold">{L(total)}</div></div>
          <div className="rounded-xl bg-secondary/60 p-3"><div className="text-[10px] uppercase tracking-wider text-muted-foreground">{t("Asubuhi", "Morning")}</div><div className="font-num font-bold">{L(morning)}</div></div>
          <div className="rounded-xl bg-secondary/60 p-3"><div className="text-[10px] uppercase tracking-wider text-muted-foreground">{t("Jioni", "Evening")}</div><div className="font-num font-bold">{L(evening)}</div></div>
          <div className="rounded-xl bg-secondary/60 p-3"><div className="text-[10px] uppercase tracking-wider text-muted-foreground">{t("Wafugaji", "Farmers")}</div><div className="font-num font-bold flex items-center gap-1"><Users className="h-3.5 w-3.5" />{uniqueFarmers}</div></div>
        </div>
        <div className="mt-5">
          <div className="text-xs font-semibold mb-2">{t("Daftari kamili", "Full intake log")}</div>
          <table className="w-full text-sm">
            <thead><tr className="text-left text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border"><th className="py-2">{t("Mfugaji", "Farmer")}</th><th>{t("Kipindi", "Session")}</th><th className="text-right">{t("Litre", "Litres")}</th></tr></thead>
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="border-b border-border last:border-0">
                    ...
                    <td className="py-2 text-xs">{r.session === "morning" ? t("Asubuhi", "Morning") : t("Jioni", "Evening")}</td>
...
function TransferDialog({ from, onSave }: { from: string; onSave: (tr: { from: string; to: string; litres: number }) => void }) {
  const { t } = useApp();
        ...
        <div className="grid gap-3">
          ...
          <div className="grid gap-1.5"><Label>{t("Kwenda", "To")}</Label>
            <Select value={to} onValueChange={setTo}>
              ...
              <SelectContent>
                {destinations.map((l) => <SelectItem key={l.id} value={l.id}>{l.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
